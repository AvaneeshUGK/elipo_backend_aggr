...
def postDefaultMasterDetail(event, context):
    print(event)
    global dbScehma 
    dbScehma = ' DBADMIN '
    ...
    try:
        ...
        with mydb.cursor() as mycursor:
            ...
            sqlQuery = "Insert into default_master (company_code,country,payment_terms,payment_method,currency,gl_account_header,cost_center,tax_per,tds_per,gl_account_item," \
            "plant,supplier_id,date_range,document_type) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?) "
            values = (record["company_code"],record["country"],record["payment_terms"],record["payment_method"], \
                record["currency"],record["gl_account_header"],record["cost_center"],record["tax_per"],record["tds_per"], \
                record["gl_account_item"],record["plant"],record["supplier_id"],record["date_range"],record["document_type"])
...
def postRuleDetails(event , context):
    ...
    try:
        ...
        with mydb.cursor() as mycursor:
            ...
            mycursor.execute("select  rule_id from rule_snro   order by rule_id desc limit 1 ")
            ...
            for row in criteria:
                ...
                if record["due_notification"] == '':
                    record["due_notification"] = 0
                if record["due_reminder"] == '':
                    record["due_reminder"] = 0
                if record["overdue_notification"] == '':
                    record["overdue_notification"] = 0
                if record["overdue_reminder"] == '':
                    record["overdue_reminder"] = 0
                if record["escelator"] == '':
                    record["escelator"] = 0
                if record["ifnot_withindays"] == '':
                    record["ifnot_withindays"] = 0
            ...
            if "rule_id" in event["params"]["querystring"]:
                ...
                if data:
                    ...
                    for row in data:
                        ...
                        if row["ifnot_withindays"] == '':
                            row["ifnot_withindays"] = 0

                        if row["due_notification"] == '':
                            row["due_notification"] = 0
                        
                        if row["due_reminder"] == '':
                            row["due_reminder"] = 0
                        
                        if row["overdue_notification"] == '':
                            row["overdue_notification"] = 0

                        if row["overdue_reminder"] == '':
                            row["overdue_reminder"] = 0
                ...
                for index, each in enumerate(approvers):
                    if each["level"] == '':
                        each["level"] = 0
                    tup = (int(rule), each["isgroup"], each["approver"], each["level"])
                    values.append(tup)
...
def downloadAttachments(event, context):
    ...
    try:
        ...
        s3 = boto3.client("s3",aws_access_key_id='AKIAXUCMAX6S27NZCRFL',
            aws_secret_access_key='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')

        file_name = event["params"]["querystring"]["file_name"]

        if "email" in event["params"]["querystring"]:  # changed
        ...
        bucket_used = event["stage-variables"]["non_ocr_attachment"]
        ...
        if "download" in event["params"]["querystring"]:
            download = True

        if "bucket" in event["params"]["querystring"]:
            bucket_name = event["params"]["querystring"]["bucket"]
