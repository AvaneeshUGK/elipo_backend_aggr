...
function_names = {
   ...
   # "account-recivable": {"GET": "getAccountsRecivableOverviewSup"},
    ...
    "svendor": {"GET": "getVendorSearchHelp"},
...
#     },
   ...
   # "inbox": {"POST": "getInboxApprovals"},
    ...
    "delete-invoice": {"DELETE": "deleteInvoice"},
    "duplicate-invoice": {"GET": "checkInvoiceAndVendor"},
    "fetch-invoice-sup": {"POST": "getSearchedInvoiceSup"},
...
def call_function(resource_name):
    ...
    try:
        ...
        if 'file' in request.files:
            event["body"] = request.files['file'].read()
            event["headers"] = header
            event["form"] = dict(request.form)
        ...
        if "isBase64Encoded" in result:
            if result["isBase64Encoded"] == True:
                return send_file(io.BytesIO(result["body"]),download_name=result["headers"]["filename"],mimetype=result["headers"]["Content-Type"],as_attachment=True)
